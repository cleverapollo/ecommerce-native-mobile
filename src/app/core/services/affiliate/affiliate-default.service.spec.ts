...
describe('AffiliateDefaultService', () => {
  ...
  describe('createAffiliateLink', () => {
    ...
    it('creates affiliate link for unknown url', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
      const link = 'https://www.mmoga.de/Rockstar-Games/Red-Dead-Redemption-2-PC-Version.html';
      const expectedResult = 'http://redirect.viglink.com?key=e0562bb177d75be5331216a6971d6bca&u=https%3A%2F%2Fwww.mmoga.de%2FRockstar-Games%2FRed-Dead-Redemption-2-PC-Version.html';
      service.createAffiliateLink(link).then(result => {
        expect(result).toBe(expectedResult);
      });
    }))

    it('creates affiliate link for otto.de', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for pearlco.de', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for nordicnest.de', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for vertbaudet.de', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for aromamanufaktur.com', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for hempamed.de', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for klaraseats.com', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
    ...
    it('creates affiliate link for amazon.de product url with existing query params', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
      const link = 'https://www.amazon.de/Mundschutzmaske-zertifiziert-Atemschutzmasken-hygienische-Einzelverpackung/dp/B08TTYBD3K/ref=zg-bs_sports_1/257-9050733-2865403?pd_rd_w=mB31y&pf_rd_p=f132fd53-9870-459e-8ea1-2187701626ae&pf_rd_r=RXNXS96YT49E0314YSRZ&pd_rd_r=dba2a0eb-efd0-4811-b14d-0a9001b46ad3&pd_rd_wg=fEb7L&pd_rd_i=B08TTYBD3K&psc=1';
      const expectedResult = 'https://www.amazon.de/Mundschutzmaske-zertifiziert-Atemschutzmasken-hygienische-Einzelverpackung/dp/B08TTYBD3K/ref=zg-bs_sports_1/257-9050733-2865403?pd_rd_w=mB31y&pf_rd_p=f132fd53-9870-459e-8ea1-2187701626ae&pf_rd_r=RXNXS96YT49E0314YSRZ&pd_rd_r=dba2a0eb-efd0-4811-b14d-0a9001b46ad3&pd_rd_wg=fEb7L&pd_rd_i=B08TTYBD3K&psc=1&tag=wantic03-21';
      service.createAffiliateLink(link).then(result => {
        expect(result).toBe(expectedResult); // 'should add affiliate tag to url using & seperator'
      });
    }))

    it('creates affiliate link for amazon.de product url without query params', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
      const link = 'https://www.amazon.de/some-product/';
      const expectedResult = 'https://www.amazon.de/some-product/?tag=wantic03-21'
      service.createAffiliateLink(link).then(result => {
        expect(result).toBe(expectedResult); // should add affiliate tag to url using ? seperator
      });
    }))

    it('creates affiliate link for amazon.de product url with existing affiliate tag from another company', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
      const link = 'https://www.amazon.de/Super-Sparrow-Trinkflasche-Wasserdurchfluss-Wiederverwendbare/dp/B07BJFX6MC/ref=zg-bs_sports_4/257-9050733-2865403?pd_rd_w=75taz&pf_rd_p=f132fd53-9870-459e-8ea1-2187701626ae&pf_rd_r=8G79P6T63YJDSYBXHJKP&pd_rd_r=39ee585c-ec54-492e-bc94-52944c897787&pd_rd_wg=1oggQ&pd_rd_i=B08K6VQNKB&psc=1&tag=some-other-tag';
      const expectedResult = 'https://www.amazon.de/Super-Sparrow-Trinkflasche-Wasserdurchfluss-Wiederverwendbare/dp/B07BJFX6MC/ref=zg-bs_sports_4/257-9050733-2865403?pd_rd_w=75taz&pf_rd_p=f132fd53-9870-459e-8ea1-2187701626ae&pf_rd_r=8G79P6T63YJDSYBXHJKP&pd_rd_r=39ee585c-ec54-492e-bc94-52944c897787&pd_rd_wg=1oggQ&pd_rd_i=B08K6VQNKB&psc=1&tag=wantic03-21';
      service.createAffiliateLink(link).then(result => {
        expect(result).toBe(expectedResult); // should override affiliate tag from another company in url
      });
    }))

    it('creates affiliate link for amazon.de product url with existing affiliate tag from wantic', waitForAsync(() => {
      const service = configureModule(affiliateProgrammes);
      const link = 'https://www.amazon.com/AmazonBasics-NC1406118R1-Laptop-Tasche-Bildschirmdiagonale-Schwarz/dp/B00LU7B8X0?ref_=ast_sto_dp&th=1&psc=1&tag=wantic03-21';
      const expectedResult = 'https://www.amazon.com/AmazonBasics-NC1406118R1-Laptop-Tasche-Bildschirmdiagonale-Schwarz/dp/B00LU7B8X0?ref_=ast_sto_dp&th=1&psc=1&tag=wantic03-21';
      service.createAffiliateLink(link).then(result => {
